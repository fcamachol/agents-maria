...
const SUBCATEGORY_MAP: Record<string, string> = {
    // CTR → TRA
    "CTR-001": "TRA-NUE", "CTR-002": "TRA-NUE", "CTR-003": "TRA-FRC",
    "CTR-004": "TRA-CTI", "CTR-005": "TRA-ADD", "CTR-006": "FAC-TAR",
    "CTR-007": "FAC-TAR", "CTR-008": "TRA-NUE", "CTR-009": "TRA-SUS",
    "CTR-010": "TRA-CAN", "CTR-011": "TRA-NUE", "CTR-012": "TRA-NUE",
    "CTR-013": "TRA-NUE", "CTR-014": "TRA-NUE",
    // CVN → FAC
    "CVN-001": "FAC-CON", "CVN-002": "FAC-CON", "CVN-003": "FAC-CNL",
    "CVN-004": "FAC-CON", "CVN-005": "INF-PRO", "CVN-006": "INF-PRO",
    "CVN-007": "INF-PRO",
    // CON → INF
    "CON-001": "INF-GEN", "CON-002": "FAC-EST", "CON-003": "INF-OFI",
    "CON-004": "INF-REQ", "CON-005": "INF-EST",
    // FAC (subcat remap)
    "FAC-001": "FAC-DIG", "FAC-002": "FAC-REC", "FAC-003": "FAC-REC",
    "FAC-004": "FAC-ACL", "FAC-005": "FAC-AJU", "FAC-006": "FAC-EST",
    "FAC-007": "FAC-PAG", "FAC-008": "FAC-AJU", "FAC-009": "FAC-ACL",
    // CNS → INF
    "CNS-001": "INF-GEN", "CNS-002": "INF-GEN", "CNS-003": "INF-GEN",
};
...
async function getCategoryId(code: string): Promise<number | null> {
    const dbCode = CATEGORY_MAP[code] || code;
    try {
        ...
        if (dbCode !== code) {
            console.log(`[getCategoryId] Mapped skill code ${code} → DB code ${dbCode}`);
        }
        return result?.[0]?.id || null;
    } catch (error) {
        console.warn(`[getCategoryId] Failed to lookup category ${code} (DB: ${dbCode}):`, error);
        return null;
    }
}

async function getSubcategoryId(code: string): Promise<number | null> {
    const dbCode = SUBCATEGORY_MAP[code] || code;
    try {
        const result = await pgQuery<{ id: number }>(
        ...
        if (dbCode !== code) {
            console.log(`[getSubcategoryId] Mapped skill code ${code} → DB code ${dbCode}`);
        }
        return result?.[0]?.id || null;
    } catch (error) {
        console.warn(`[getSubcategoryId] Failed to lookup subcategory ${code} (DB: ${dbCode}):`, error);
        return null;
    }
}
...
export async function createTicketDirect(input: CreateTicketInput): Promise<CreateTicketResult> {
    ...
    try {
        ...
        const convId = input.conversation_id || process.env.CURRENT_CHATWOOT_CONVERSATION_ID || '';
        ...
        const conversationId = input.conversation_id || parseInt(process.env.CURRENT_CHATWOOT_CONVERSATION_ID || '') || null;
            ...
            return {
                content: [{
                    ...
                    text: JSON.stringify({
                        ...
                        formatted_response: "No pude consultar tu historial de consumo en este momento. ¿Puedes intentar en unos minutos?"
                    })
                }]
            };
        }
