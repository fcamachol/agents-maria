...
import { staticTools, createContextTools, getVerifiedContracts, translateContract } from "./tools.js";
...
12. EVIDENCIA FOTOGRÁFICA:
    - Para REPORTES (fugas, drenaje, calidad): la foto es OPCIONAL, ofrécela pero no la exijas
    ...
    - Si el usuario dice que no tiene foto, continúa el flujo normalmente
...
16. DOCUMENTOS Y ARCHIVOS (PDF, DOC, XLS, etc.):
    ...
    - Si el usuario envía un documento, responde: "Por el momento no puedo abrir documentos, ¿me puedes mandar una foto?"
    - Si el usuario dice que no puede o no quiere enviar foto, usa handoff_to_human con motivo "El usuario envió un documento que no puedo procesar y no puede enviar foto"

17. FOLIOS DE TICKETS (REGLA ABSOLUTA — MÁS IMPORTANTE QUE TODAS LAS DEMÁS):
    - NUNCA generes, inventes o escribas un número de folio (CEA-XXXXX-XXXXX)
    - Los folios SOLO existen dentro del resultado JSON de create_ticket o get_client_tickets
    - Si NO llamaste create_ticket en ESTE turno, NO puedes decir que creaste un reporte
    - Si create_ticket falló, di: "No pude crear tu reporte, ¿quieres que lo intente de nuevo?"
    - Si ves un folio CEA-XXXXX en el historial, ese folio pertenece a un ticket YA CREADO — NUNCA lo copies para un nuevo reporte
    - Cada reporte genera su propio folio ÚNICO vía create_ticket — no hay forma de saber el folio sin llamar la herramienta
`;
...
export async function runWorkflow(input: WorkflowInput): Promise<WorkflowOutput> {
    ...
    // Build per-request context — closures guarantee correct values even under concurrency
    ...
    try {
        ...
        if (inputLower.includes("fuga") || inputLower.includes("no hay agua") ||
            ...
            category = "SRV";
            keywordMatched = true;
            // Flag "reportar lectura" context so media pipeline skips detailed medidor analysis
            if (inputLower.includes("lectura")) {
        ...
        // Classification fallback: if keyword matched CON but conversation was in a
        // more specific category, keep previous — likely a follow-up, not a topic switch
        if (keywordMatched && category === "CON" && conversation.category && conversation.category !== "CON") {
            console.log(`[Workflow] Classification fallback: "${category}" → "${conversation.category}" (continuing conversation)`);
        ...
        // Strip all image/media analysis sections to avoid picking up meter readings or serial numbers
        ...
        if (contractMatch) {
            ...
            // Old SIGE contracts are 9 digits, new Hydra contracts are 6 digits
            if (extractedContract.length === 9) {
                const translated = await translateContract(extractedContract);
                if (translated) {
                    console.log(`[Workflow] Translated SIGE contract ${extractedContract} → Hydra ${translated}`);
                    conversation.contractNumber = translated;
                    extractedContract = translated;
                } else {
                    console.log(`[Workflow] SIGE contract ${extractedContract} not found in Hydra mapping`);
        ...
        // Enforce least-privilege: only provide tools the skill needs
        const skillToolNames = new Set(skill.tools);
        // Context tools are always available
        const contextToolNames = new Set(["create_ticket", "handoff_to_human", "validate_contract_holder"]);
        const scopedTools = allTools.filter(t =>
            skillToolNames.has(t.name) || contextToolNames.has(t.name)
        );

        console.log(`[Workflow] Skill ${skill.code} gets ${scopedTools.length}/${allTools.length} tools: [${scopedTools.map(t => t.name).join(', ')}]`);
...
${input.input_as_text}`;
        ...
        let mcpServerConfig;
        try {
            mcpServerConfig = createSdkMcpServer({
                name: "maria-cea-tools",
                version: "1.0.0",
                tools: scopedTools
            });
        } catch (e) {
            console.error(`[Workflow] MCP server creation FAILED:`, e);
            return {
                output_text: "Estoy teniendo problemas técnicos. ¿Podrías intentar de nuevo en unos minutos?",
                output_messages: ["Estoy teniendo problemas técnicos. ¿Podrías intentar de nuevo en unos minutos?"],
                error: e instanceof Error ? e.message : "MCP server creation failed",
                toolsUsed: [],
                category
            };
        }
        ...
        const result = query({
            ...
            options: {
                ...
                maxBudgetUsd: category === "REP" || category === "SRV" ? 1.00 : 0.50,
        ...
        for await (const message of result) {
            if (message.type === "assistant") {
                ...
                if ((message as any).error) {
                    console.error(`[Workflow] SDK error on assistant message: ${JSON.stringify((message as any).error)}`);
                }
                let turnText = "";
                ...
                if (Array.isArray(content)) {
                    for (const block of content) {
                        if (block.type === "text") {
                            ...
                            // SDK returns MCP-prefixed names like "mcp__maria-cea-tools__create_ticket"
                            // Strip prefix to get bare tool name for guard checks
                ...
                // Skip text from turns that have tool calls — it's just preamble
                ...
                if ((message as any).subtype === "error_max_budget_usd") {
                    console.warn(`[Workflow] BUDGET EXHAUSTED — response may be incomplete`);
                }
            }
        }
        ...
        // Guard: detect hallucinated folios
        const FOLIO_AWARE_TOOLS = new Set([
            "create_ticket",
            "get_client_tickets",
            "update_ticket",
            "handoff_to_human",
        ...
        const hasFolioPattern =
            ...
            /\b(REP|FAC|SRV|TRA|INF)[-_]\d{3,}\b/i.test(output) ||
            /\b(REP|FAC|SRV|TRA|INF)-[A-Z]{2,4}-\d{6,}-\d{3,}\b/i.test(output) ||
            /\bfolio\s*[#:]?\s*\d{4,}/i.test(output);
        const calledFolioAwareTool = toolsUsed.some(t => FOLIO_AWARE_TOOLS.has(t));

        // Check if ALL CEA folios in output are known-good (created in previous turns)
        const outputFolios = output.match(/\bCEA-\d{4,8}-\d{3,6}\b/gi) || [];
        const allFoliosKnown = outputFolios.length > 0 && outputFolios.every(f => conversation.createdFolios.has(f));

        if (hasFolioPattern && !calledFolioAwareTool && !allFoliosKnown) {
            const hallucinationType = toolsUsed.length === 0 ? "NO_TOOLS" : "WRONG_TOOLS";
            console.warn(`[Workflow] HALLUCINATION_BLOCKED [${hallucinationType}]: Tools: [${toolsUsed.join(", ")}]. Output: "${output.substring(0, 300)}"`);

            const claimedCreation = /cre[eé]|registr[eé]|levant[eé]|tu reporte.*folio|ticket.*creado/i.test(output);
            if (claimedCreation) {
                output = "No pude registrar tu reporte en este momento. Voy a intentar crearlo, ¿podrías repetir tu solicitud?";
            } else {
                // Strip hallucinated folios but preserve message
                output = output
                    .replace(/\bCEA-\d{4,8}-\d{3,6}\b/gi, "")
                    .replace(/\b(REP|FAC|SRV|TRA|INF)[-_]\d{3,}\b/gi, "")
                    .replace(/con folio\s*/gi, "")
                    .replace(/\s{2,}/g, " ").trim();
                if (!output) output = "Hubo un problema con mi respuesta. ¿Podrías repetir tu solicitud?";
            }
            outputMessages.length = 0;
            outputMessages.push(output);
        } else if (hasFolioPattern && calledFolioAwareTool) {
            console.log(`[Workflow] Folio in output ALLOWED — tools: [${toolsUsed.join(", ")}]`);
        } else if (allFoliosKnown) {
            console.log(`[Workflow] Folio in output ALLOWED — matches known folio from previous turn`);
        }

        // Track folios created in this turn for future reference
        if (toolsUsed.includes("create_ticket")) {
            const folioMatches = output.match(/\bCEA-\d{4,8}-\d{3,6}\b/gi);
            if (folioMatches) {
                for (const folio of folioMatches) {
                    conversation.createdFolios.add(folio);
                }
            }
        }

        // Zero-tool safety net for ticket-creating categories
        const TICKET_CATEGORIES = new Set(["REP", "SRV", "FAC", "CTR", "CVN"]);
        if (toolsUsed.length === 0 && TICKET_CATEGORIES.has(category) && outputMessages.length > 0) {
            console.warn(`[Workflow] ZERO_TOOLS for ${category} category — possible MCP failure or model not calling tools`);
        }
