...
import { verifyReciboToken } from "./tools.js";
import { apiKeyAuth } from "./middleware/auth.js";
import { chatLimiter, webhookLimiter, chatwootLimiter } from "./middleware/rate-limit.js";
import { fetchReciboPdf } from "./services/cea-api.js";
...
const REQUIRED_VARS = ['ANTHROPIC_API_KEY', 'PGHOST', 'PGPASSWORD', 'CHATWOOT_API_TOKEN'];
 ...
 * Main chat endpoint
 */
app.post("/api/chat", chatLimiter, apiKeyAuth, async (req, res) => {
    try {
 ...
 */
app.post("/webhook", webhookLimiter, apiKeyAuth, async (req, res) => {
    try {
        const body = req.body;
 ...
 *
 * Per-conversation serial queue: messages for the same conversation are
 * processed sequentially to prevent ordering bugs. Different conversations
 * run concurrently.
 */
const conversationLocks = new Map<number, Promise<void>>();

app.post("/chatwoot", chatwootLimiter, async (req, res) => {
    ...
    const currentLock = previousLock.then(async () => {
        try {
            ...
            // WhatsApp sends the document filename as payload.content (e.g., "factura.pdf")
...
app.use(apiKeyAuth, internalRoutes);
