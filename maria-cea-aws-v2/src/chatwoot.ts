...
const CHATWOOT_USER_API_TOKEN = process.env.CHATWOOT_USER_API_TOKEN || "";
...
export function shouldProcessWebhook(payload: ChatwootWebhookPayload): {
    ...
    console.log(`[Chatwoot] Payload: event=${payload.event}, conv=${payload.conversation?.id}, sender_id=${payload.sender?.id}, msg_type=${payload.message_type}`);
...
export async function processAttachments(
    attachments: ChatwootAttachment[],
    pendingLectura = false
): Promise<string> {
    // Process media attachments (audio, images) with AI
    const mediaResult = await processMediaAttachments(attachments, pendingLectura);
    ...
    for (const attachment of attachments) {
        ...
        switch (attachment.file_type) {
            ...
            case "file":
                parts.push(`[El usuario envió un documento (${attachment.extension || "documento"}). NO puedes abrir ni leer documentos. Debes responder: "Por el momento no puedo abrir documentos, ¿me puedes mandar una foto?" Si el usuario responde que no puede o no quiere enviar foto, usa handoff_to_human.]`);
                break;
...
export async function updateConversationStatus(
    ...
    const token = CHATWOOT_USER_API_TOKEN || CHATWOOT_API_TOKEN;
    if (!CHATWOOT_BASE_URL || !token) {
        console.error("[Chatwoot] Missing CHATWOOT_BASE_URL or API token");
        return { success: false, error: "Chatwoot not configured" };
    }
    ...
    try {
        console.log(`[Chatwoot] Toggling conversation ${conversationId} status to: ${status}`);
        ...
        if (!response.ok) {
            const errorText = await response.text();
            console.error(`[Chatwoot] Status toggle error ${response.status}: ${errorText}`);
            return { success: false, error: `API error: ${response.status}` };
        }

        const data = await response.json() as { payload?: { current_status?: string } };
        console.log(`[Chatwoot] Conversation ${conversationId} status toggled to: ${data?.payload?.current_status || status}`);
        return { success: true };
    } catch (error) {
        console.error("[Chatwoot] Status toggle error:", error);
        return {
            success: false,
            error: error instanceof Error ? error.message : "Unknown error"
        };
