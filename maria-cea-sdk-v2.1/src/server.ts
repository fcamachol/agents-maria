...
import { normalizePhone, recordEvent } from "./session/index.js";
...
app.get("/recibo/:contrato", async (req, res) => {
    ...
    try {
        ...
        const pdfBuffer = await fetchReciboPdf(contrato, info.explotacion, numFactura);
...
async function flushBuffer(convId: number): Promise<void> {
    ...
    const currentLock = previousLock.then(async () => {
        try {
            ...
            // Cross-channel producer: record this inbound WhatsApp message in the
            // shared session hub so a live voice call can pick it up (revisar_whatsapp).
            // Additive + fail-safe: never affects the text reply.
            try {
                const phoneKey = normalizePhone(context.senderPhone);
                if (phoneKey) {
                    const anyMedia = messages.some(m => m.hasMedia);
                    await recordEvent(phoneKey, {
                        channel: "whatsapp",
                        type: anyMedia ? "media" : "message",
                        payload: { summary: combinedText.substring(0, 500), hasMedia: anyMedia },
                        ts: Date.now()
                    });
                }
            } catch (e) {
                console.error("[Chatwoot] recordEvent (cross-channel) failed (non-fatal):", e);
            }
