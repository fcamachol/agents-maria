...
async function analyzeMedidor(base64Data: string, _mediaType: ImageMediaType): Promise<string | null> {
    console.log(`[Media] Pass 2: Analyzing medidor via Gemini Vision webhook...`);
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`[Media] Medidor webhook returned HTTP ${response.status}, falling back to format`);
            return `[ANÁLISIS DE MEDIDOR]:\n- Lectura: no disponible (error en análisis)\n- Confianza: baja\n- Estado físico: no determinado`;
        }

        const data = await response.json() as Record<string, any>;
        const lectura = data.lectura_medidor || {};
        const info = data.informacion_medidor || {};
        const estado = data.estado_medidor || {};
        const meta = data.metadata || {};

        console.log(`[Media] Medidor webhook result: lectura=${lectura.valor_lectura}, confianza=${meta.confianza_lectura}, modelo=${data.proceso?.modelo}`);

        // Format the response in the same [ANÁLISIS DE MEDIDOR] format the skills expect
        return `[ANÁLISIS DE MEDIDOR]:
- Lectura: ${lectura.valor_lectura || "no visible"} m³ (confianza: ${meta.confianza_lectura || "baja"})
- Dígitos enteros: ${lectura.digitos_enteros || "no visible"}
- Dígitos decimales: ${lectura.digitos_decimales || "no visible"}
- Serie: ${info.numero_serie || "no visible"}
- Marca: ${info.marca || "no visible"}
- Modelo: ${info.modelo || "no visible"}
- Tipo: ${info.tipo_medidor || "no determinable"}
- Diámetro: ${info.diametro || "no visible"}
- Estado físico: ${estado.condicion_general || "no determinable"}
- Legibilidad: ${estado.legibilidad || "no determinable"}
- Vidrio: ${estado.vidrio || "no visible"}
- Carcasa: ${estado.carcasa || "no visible"}
- Sellos: ${estado.sellos || "no visible"}
- Observaciones: ${estado.observaciones || "ninguna"}
- Calidad de imagen: ${meta.calidad_imagen || "no evaluada"}
- Notas: ${meta.notas || "ninguna"}`;
    } catch (error) {
        console.error(`[Media] Medidor webhook error:`, error);
        return `[ANÁLISIS DE MEDIDOR]:\n- Lectura: no disponible (error en análisis)\n- Confianza: baja\n- Estado físico: no determinado`;
    }
}
...
async function analyzeRecibo(base64Data: string, mediaType: ImageMediaType): Promise<string | null> {
    ...
    return callClaudeVision(base64Data, mediaType, `Eres un especialista en facturación de la CEA Querétaro. Analiza esta imagen de recibo o factura de agua y extrae TODA la información visible.
    ...
    for (const attachment of attachments) {
        ...
        switch (attachment.file_type) {
            ...
            case "image": {
                ...
                try {
                    if (category === "MEDIDOR" && !skipMedidorAnalysis) {
            ...
            case "video": {
                ...
                try {
                    ...
                    } else if (videoCategory === "MEDIDOR" && !skipMedidorAnalysis) {
