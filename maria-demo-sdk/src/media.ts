...
// analyzers for CEA-specific content
//
...
async function classifyVideo(base64Data: string, mimeType: string): Promise<string | null> {
    ...
    return callGeminiVideo(base64Data, mimeType, `Eres un asistente de la CEA Querétaro (agua y drenaje). Analiza este video y responde con el siguiente formato:
...
- RECIBO: Video de recibo o factura de la CEA
- NO_RELACIONADO: Cualquier video que NO muestre un problema de agua, drenaje, infraestructura hídrica, medidor ni recibo de la CEA
...
async function analyzeVideoFuga(base64Data: string, mimeType: string): Promise<string | null> {
    ...
    return callGeminiVideo(base64Data, mimeType, `Eres un inspector técnico de la CEA Querétaro especializado en fugas de agua y drenaje. Analiza este video con máximo detalle para que el equipo de campo pueda actuar sin necesidad de más información.
...
async function analyzeVideoMedidor(base64Data: string, mimeType: string): Promise<string | null> {
    ...
    return callGeminiVideo(base64Data, mimeType, `Eres un técnico especialista en medidores de agua de la CEA Querétaro. Analiza este video de medidor y extrae TODA la información visible.
...
async function analyzeVideoRecibo(base64Data: string, mimeType: string): Promise<string | null> {
    ...
    return callGeminiVideo(base64Data, mimeType, `Eres un especialista en facturación de la CEA Querétaro. Analiza este video de recibo o factura de agua y extrae TODA la información visible.
...
async function classifyImage(base64Data: string, mediaType: ImageMediaType): Promise<string | null> {
    ...
    return callClaudeVision(base64Data, mediaType, `Eres un asistente de la CEA Querétaro (agua y drenaje). Analiza esta imagen y responde con el siguiente formato:
...
async function analyzeMedidor(base64Data: string, mediaType: ImageMediaType): Promise<string | null> {
    ...
    return callClaudeVision(base64Data, mediaType, `Eres un técnico especialista en medidores de agua de la CEA Querétaro. Analiza esta foto de medidor y extrae TODA la información visible.
...
async function analyzeFuga(base64Data: string, mediaType: ImageMediaType): Promise<string | null> {
    ...
    return callClaudeVision(base64Data, mediaType, `Eres un inspector técnico de la CEA Querétaro especializado en fugas de agua y drenaje. Analiza esta foto con máximo detalle para que el equipo de campo pueda actuar sin necesidad de más información.
...
const RECIBO_EXTRACTION_PROMPT = `You are an enterprise-grade OCR and data extraction system specializing in recibos de agua de la CEA Querétaro (Comisión Estatal de Aguas). Your task is to extract billing data from the provided bill image.

=== STRICT EXTRACTION RULES ===
1. DOCUMENT VALIDATION (ANTI-ABUSE): Before extracting any data, verify that the document is indeed a Comisión Estatal de Aguas (CEA) Querétaro bill. If it is NOT, abort the process immediately, ignore all other rules, and output exactly and ONLY this phrase: "Este no es un recibo de agua de la cea."
2. OUTPUT FORMAT: If the document IS a valid CEA bill, you must output ONLY a valid, parseable JSON object. Do NOT wrap the response in markdown blocks (e.g., \`\`\`json). Do NOT include any conversational text before or after the JSON.
3. NEGATIVE CONSTRAINTS (NOISE FILTERING): You must completely ignore all handwritten notes, manual signatures, bank teller stamps, and "PAGADO" (paid) watermarks or stamps. Extract ONLY the original machine-printed text generated by the CEA billing system.
4. DATA SANITIZATION: For fields expecting a "number", strip all currency symbols ($), commas (,), and whitespace. Return pure floats/integers.
5. DATES: Extract dates exactly as they appear in the printed text.
6. MISSING/ILLEGIBLE DATA: If a field is not present in the original printed text, or is completely obscured by a stamp to the point of being illegible, return null. Do not guess.

=== TARGET JSON SCHEMA ===
Use the exact keys and data types outlined below:
...
const RECIBO_REJECTION_SENTENCE = "Este no es un recibo de agua de la cea.";
...
function formatM3(v: unknown): string | null {
    if (!isPresent(v)) return null;
    return `${v} m³`;
}

// Adapter: turn the new structured-JSON extraction back into the labeled block
// shape Maria already knows. Skips lines whose source field is null so the
// per-turn token cost stays small on partial extractions.
function formatReciboAsLabeledBlock(raw: string | null): string {
    const header = "[ANÁLISIS DE RECIBO]:";

    if (!raw) {
        return `${header}\nNo se pudo extraer información estructurada del recibo. Salida cruda: (vacía)`;
    }

    const trimmed = raw.trim();

    if (trimmed.includes(RECIBO_REJECTION_SENTENCE)) {
        return `${header}\nLa imagen no parece ser un recibo CEA Querétaro. Detalle: ${RECIBO_REJECTION_SENTENCE}`;
    }
    ...
    try {
        ...
        return `${header}\nNo se pudo extraer información estructurada del recibo. Salida cruda: ${trimmed.slice(0, 500)}`;
    }

    const lines: string[] = [header];

    const top: [string, unknown][] = [
        ["Contrato", json?.datosCliente?.numeroContrato],
        ["Titular", json?.datosCliente?.nombre],
        ["Dirección", json?.datosCliente?.direccion],
        ["Periodo", json?.datosMedidor?.periodoConsumo],
        ["Monto total", formatMoney(json?.datosFacturacion?.totalAPagar)],
        ["Fecha de vencimiento", json?.datosFacturacion?.fechaVencimiento],
        ["Lectura anterior", formatM3(json?.datosMedidor?.lecturaAnterior)],
        ["Lectura actual", formatM3(json?.datosMedidor?.lecturaActual)],
        ["Consumo", formatM3(json?.datosMedidor?.consumoM3)],
    ];
    for (const [label, value] of top) {
        if (isPresent(value)) lines.push(`- ${label}: ${value}`);
    }

    const cfdi: [string, unknown][] = [
        ["Número de factura", json?.datosFacturacion?.numeroFactura],
        ["Referencia electrónica", json?.datosFacturacion?.referenciaElectronica],
        ["UUID fiscal", json?.datosFiscales?.folioFiscalUUID],
        ["Subtotal", formatMoney(json?.datosFacturacion?.subtotal)],
        ["IVA", formatMoney(json?.datosFacturacion?.iva)],
        ["Adeudo pendiente", formatMoney(json?.datosFacturacion?.montoFacturasPendientes)],
        ["Meses de adeudo", json?.datosFacturacion?.mesesAdeudo],
    ];
    const cfdiLines = cfdi
        .filter(([, v]) => isPresent(v))
        .map(([label, value]) => `- ${label}: ${value}`);

    const conceptos = Array.isArray(json?.conceptos) ? json.conceptos : [];
    const conceptoLines = conceptos
        .filter((c: { descripcion?: unknown; importe?: unknown; valorUnitario?: unknown }) =>
            isPresent(c?.descripcion) || isPresent(c?.importe) || isPresent(c?.valorUnitario))
        .map((c: { descripcion?: unknown; importe?: unknown; valorUnitario?: unknown }) => {
            const importe = isPresent(c?.importe) ? `$${c.importe}` : "?";
            const valor = isPresent(c?.valorUnitario) ? ` (valor unitario $${c.valorUnitario})` : "";
            const desc = isPresent(c?.descripcion) ? String(c.descripcion) : "(sin descripción)";
            return `  · ${desc}: ${importe}${valor}`;
        });

    if (cfdiLines.length > 0 || conceptoLines.length > 0) {
        lines.push("");
        lines.push("Datos adicionales (CFDI):");
        lines.push(...cfdiLines);
        if (conceptoLines.length > 0) {
            lines.push("- Conceptos:");
            lines.push(...conceptoLines);
        }
    }
...
async function analyzeRecibo(base64Data: string, mediaType: ImageMediaType): Promise<string | null> {
    ...
    const raw = await callClaudeVision(base64Data, mediaType, RECIBO_EXTRACTION_PROMPT, 1500);
    return formatReciboAsLabeledBlock(raw);
}
